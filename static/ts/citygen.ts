class CityGenerator {
    ...
    MaxPopVariance: number = 0.3
    City: City
    Races: IRaceFrequencyGroup
    TotalPopSize: number = 1
    Mine: Business
    LumberCamp: Business
    NameGenerators: NameGenerator[] = []

    constructor(settings: CitySettings) {
    ...
    getCitySizeByName(name: string): CitySize {
    ...
    createPeopleByCaste(caste: Caste, count: number) {
    ...
    private addFamily(family: Family) {
    ...
    createNewBusiness(businessType: BusinessType, owner: Person) {
    ...
    setInventory(business: Business) {
    ...
    adjustAgeByRace(race: Race, age: number) {
        switch (race.name) {
